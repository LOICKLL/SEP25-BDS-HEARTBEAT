"""
Random Forest binaire sur MIT-BIH (lead MLII, split patient-level)
...
# ----------------------------
# 3) RandomizedSearchCV Random Forest (GroupKFold)
# ----------------------------
def tune_rf_binary(X_train, y_train, sample_weights, groups_train):
    rf_base = RandomForestClassifier(
        n_jobs=-1,
        random_state=42,
        class_weight=None,  # on passe sample_weight explicitement
    )
    ...
    print("\n=== Recherche d'hyperparamètres RF (binaire, score = F1, GroupKFold) ===")
...
def main():
    ...
    best_model, best_params, best_cv_f1 = tune_rf_binary(
        X_train, y_train, sample_weights, groups_train
    )

    # Ré-entraînement sur tout le train
    print("\nRé-entraînement du meilleur Random Forest sur tout le train...")
    best_model.fit(X_train, y_train, sample_weight=sample_weights)

    # Sauvegarde modèle
    model_path = PROJECT_ROOT / "models" / "rf_mit_mlii_binary.pkl"
    model_path.parent.mkdir(parents=True, exist_ok=True)
    joblib.dump(best_model, model_path)
    print("Modèle RF sauvegardé dans :", model_path)

    # Évaluation sur le test (patients jamais vus)
    print("\n=== Évaluation RF sur le test (patients jamais vus) ===")
    ...
    print("Meilleur F1 (CV groupée RF)        :", best_cv_f1)
    ...
    # Figures
    # ----------------------------
    img_dir = PROJECT_ROOT / "reports" / "figures" / "Modelisation_RandomForest"
    ...
    plt.title("Matrice de confusion normalisée (test) - RF")
    plt.xlabel("Prédit"); plt.ylabel("Vrai")
    plt.tight_layout()
    img_cm_path = img_dir / "rf_cm_mit_test.png"
    plt.savefig(img_cm_path, dpi=150)
    plt.close()
    print(f"Matrice de confusion RF sauvegardée dans : {img_cm_path}")
    ...
    plt.ylim(0, 1.0)
    plt.title("Scores sur le test (Random Forest binaire)")
    plt.ylabel("Score")
    plt.legend(loc="lower right")
    plt.tight_layout()
    img_scores_path = img_dir / "rf_scores_mit_test.png"
    plt.savefig(img_scores_path, dpi=150)
    plt.close()
    print(f"Scores RF (precision/recall/F1) sauvegardés dans : {img_scores_path}")
    ...
    table.scale(1.2, 1.4)
    plt.title("Classification report Random Forest Binaire", pad=10)
    plt.tight_layout()
    img_report_path = img_dir / "rf_report_mit_test.png"
    plt.savefig(img_report_path, dpi=150)
    plt.close()
    print(f"Tableau de scores RF sauvegardé dans : {img_report_path}")
