...
# 3) GridSearchCV sur SVM RBF binaire


def tune_svm_binary(X_train, y_train):
    """
    ...
    svm_pipe = Pipeline(
        steps=[
            ...
            ("clf", SVC(
                ...
                probability=True,   # on garde au cas où pour des courbes ROC plus tard
                random_state=42,
            )),
        ]
    )
    ...
    print("\n=== GridSearchCV SVM RBF (score = F1) ===")
    grid.fit(X_train, y_train)

    print("\nMeilleurs hyperparamètres SVM :")
    print(grid.best_params_)

    print("\nMeilleur F1 (CV) :", grid.best_score_)
...
# 4) Fonctions de visualisation (style XGBoost)


def plot_confusion_matrix_norm(y_true, y_pred, labels, display_labels, title, out_path):
    """
    Matrice de confusion normalisée par ligne (true).
    """
...
def main():
    ...
    best_model, best_params, best_cv_f1 = tune_svm_binary(X_train, y_train)
